defmodule App.Mixfile do # this is a fairly standard/simple mix file ...
  ...
  defp deps do
    [
      ...
      {:json, "~> 1.0", only: :test}, # for reading JSON fixture file
      {:excoveralls, "~> 0.6.2", only: :test}, # tracking test coverage
      {:ex_doc, "~> 0.11", only: :test}, # to generate documentation
      {:dogma, "~> 0.1", only: :test}, # Elixir style
    ]
  end
end
